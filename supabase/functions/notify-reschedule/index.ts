 ...
 * notify-reschedule
 * ─────────────────
 * Sends an SMS to the customer when their booking is rescheduled by the business.
 *
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let template = `Hi {first_name}! Your {service} at {business} has been rescheduled.\n📅 New time: {date} at {time}\nTo cancel: {cancel_link}`
    const { data: tpl } = await supabase.from('sms_templates')
      .select('template').eq('client_id', (booking as any).client_id).eq('type', 'reschedule').maybeSingle()
